...
The Segment source-specific write key which you can find in your `analytics.load()` call in your Analytics.js snippet.
...
Callback function that determines if consent is required before tracking begins. Set to `true` to show the consent banner, otherwise return `false` to not show consent banner.
